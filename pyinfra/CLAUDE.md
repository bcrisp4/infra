...
- **Data layout**: hosts are declared once in `inventory.py`'s explicit `all` list with a per-host `_<name>_data` dict (machine facts only); role groups (`dns_servers`, `monitoring_servers`, `metrics_agents`, ...) are bare-FQDN lists; `group_data/all.py` holds every default with `<name>_enabled = False`; `group_data/<role>.py` files flip enables + role policy. Keys stay disjoint across role files. A guard in `inventory.py` raises if a role references a host missing from `all` (pyinfra silently drops those). Prometheus scrape jobs come ONLY from `prometheus_scrape_targets` in the monitoring server's host dict - enabling a service does not add its scrape job.
...
- `--dry` gathers facts as the SSH user BUT sudo-gated facts (e.g. image_renderer's `test -s` token check) still prompt for the sudo password, so `--dry` also needs a TTY (`!` prefix) or `PYINFRA_SUDO_PASSWORD`. Fully non-interactive check from Claude's Bash: `uv run pyinfra inventory.py debug-inventory` (resolved data, no facts). macOS lacks `timeout`; don't wrap pyinfra in it.
- `--dry` ALWAYS reports "Change: 1" for the `files.directory` chown ops on `/var/lib/{prometheus,grafana,bfeed}` (owner is a numeric uid with no passwd entry, so the ownership fact never matches). Not drift; the apply-time chown is a no-op. Render ops showing `-` is the real zero-change signal.
...
- Network config (`tasks/network.py`) uses the Netplan source file. Keep its UUID and profile name aligned with the active NetworkManager profile. The task removes the old static keyfile and runs `netplan apply` only when the YAML changes.
...
- podman MASKS `/sys/firmware` by default (empty tmpfs that also SHADOWS any bind-mount there). A container reading `/proc/device-tree/*` (a symlink to `/sys/firmware/devicetree/base`, e.g. Raspberry Pi board/firmware detection) gets ENOENT; no `Volume=` fixes it. Reveal the host's read-only firmware sysfs with native quadlet `Unmask=/sys/firmware` (-> `--security-opt unmask=/sys/firmware`). Device passthrough (`AddDevice=`) + group (`GroupAdd=`) are unrelated to this masking (see `tasks/pi5_exporter.py`).
