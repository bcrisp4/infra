...
- `storage()` forces every mount dir to `root:root 0700` on each run; the consuming container task (e.g. `grafana()`) re-chowns it to its container uid LATER in deploy.py. A run that aborts between them leaves the data dir root-owned and the container crash-loops (`GF_PATHS_DATA ... is not writable` / `Permission denied`). A *completed* run self-heals; keep consumers ordered after `storage()`, and chown manually to unblock a crash loop.
